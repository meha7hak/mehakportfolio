  ...
  if (initError) {
    return (
      <div
        ...
        style={{ ...style, background: '#060010' }}
      >
        {children ? <div className="laser-flow-children">{children}</div> : null}
      </div>
    );
  }

  return (
    <div ref={mountRef} className={`laser-flow-container ${className || ''}`} style={style}>
      {children ? <div className="laser-flow-children">{children}</div> : null}
    </div>
  );
};
