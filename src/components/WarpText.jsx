...
const buildTextCanvas = ({ container, width, height, dpr, props }) => {
  ...
  // Word-wrap: split each explicit line by words, building visual lines
  // that fit within maxWidth so long paragraphs never collapse to a single
  // huge line that forces `fit` to shrink the font to nothing.
  ...
  for (const rawLine of rawLines) {
    ...
    for (const word of words) {
      ...
      if (current && measureLine(ctx, candidate, letterSpacing) > maxWidth) {
        lines.push(current);
        current = word;
      } else {
        current = candidate;
